    ...
    def _create_completion(self, messages, response_format=None):
        ...
        if response_format:
            ...
            try:
                ...
                payload = response.choices[0].message.parsed.model_dump()
            except Exception:
...
class HuggingFace_Embedding(LLM):
    ...
    def __init__(
        ...
        self._batch_size = Config.get("batch_size", 32) if Config else 32

    def _encode(self, input: Embedding_message) -> Embedding_output:
        if not input:
            return []
    ...
    async def API_client_async(self, input: Embedding_message) -> Embedding_output:
        ...
        return await loop.run_in_executor(None, self._encode, input)
